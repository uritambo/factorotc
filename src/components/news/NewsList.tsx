...
export function NewsList({ articles }: { articles: NewsArticle[] }) {
  ...
  const allTickers = Array.from(new Set(articles.flatMap((a) => a.relatedTickers))).sort();
  ...
  const filtered = articles.filter((article) => {
    ...
    const matchTicker = !selectedTicker || article.relatedTickers.includes(selectedTicker);
